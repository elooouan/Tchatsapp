...
public class IncomingPacketProcessor implements PacketProcessor {
    private ContactRegistry contats;
    ...
    public IncomingPacketProcessor(Listener listener) {
        contats = Client.getClientState().getContactRegistry();
    ...
    private void handleGroupEvent(Packet pkt, ByteBuffer payload) {
        ...
        switch (subType) {
            case GroupEventType.MEMBER_ADDED -> handleMemberAdded(pkt, payload);
            case GroupEventType.MEMBER_REMOVED -> handleMemberRemoved(pkt, payload);
            case GroupEventType.RENAMED -> handleGroupRenamed(pkt, payload);
            case GroupEventType.DELETED -> handleGroupDeleted(pkt, payload);
            case GroupEventType.CREATED -> handleGroupCreated(pkt, payload);
        }
    }
     ...
     */
    private void handleMemberAdded(Packet pkt, ByteBuffer payload) {
     ...
     */
    private void handleMemberRemoved(Packet pkt, ByteBuffer payload) {
        int groupId = payload.getInt();
        int addedMember = payload.getInt();
     ...
     */
    private void handleGroupRenamed(Packet pkt, ByteBuffer payload) {
        int groupId = payload.getInt();
     ...
     */
    private void handleGroupDeleted(Packet pkt, ByteBuffer payload) {
        int groupId = payload.getInt();
     ...
     */
    private void handleGroupCreated(Packet pkt, ByteBuffer payload) {
        int groupId = payload.getInt();
        String title = readString(payload);
